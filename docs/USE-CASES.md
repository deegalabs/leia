...
Atores: **Advogado** (autenticado por link ou login simples na PoC), **Cliente** (link com token, sem cadastro),
**Verificador** (qualquer pessoa com o QR), **Sistema** (interface + serviço de LLM).

| # | Caso de uso | Ator | Fluxo principal | Resultado | Entrega |
|---|---|---|---|---|---|
| UC-01 | Enviar documento | Advogado | escolhe o tipo, envia o PDF; sistema extrai cláusulas e gera explicação com citações | documento com seções e perguntas sugeridas | V1 |
...
| UC-03 | Percorrer o documento | Cliente | abre o link, vê a apresentação da IA e os limites, avança um tópico por vez, pode ver o trecho original e ouvir (V2) | todos os tópicos vistos | V1 (texto), V2 (voz) |
| UC-04 | Tirar dúvida | Cliente | pergunta em texto (V1) ou voz (V2); resposta cita a cláusula ou informa que não está no documento e anota para o advogado | dúvida registrada | V1 |
| UC-05 | Responder às perguntas de compreensão | Cliente | responde com as próprias palavras; nota < 2 gera nova explicação e a mesma pergunta; máximo 2 tentativas | respostas e notas registradas; pendências marcadas | V1 |
| UC-06 | Confirmar entendimento | Cliente | vê a lista do que entendeu e as pendências; confirma | sessão confirmada | V1 |
