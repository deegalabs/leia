...
# ══════════════════════════════════════════════════════════════════════════
async def run_structured_summary(
    tarefa_id: int,
    pdf_bytes: Optional[bytes] = None,
    texto: Optional[str] = None,
    filename: str = "documento.pdf",
    ...
    record_event(hash_, "resumo_estruturado_start", tarefa_id=tarefa_id)
    ...
    try:
        ...
        record_event(hash_, "resumo_estruturado_erro", etapa="submit", erro=str(e))
        return
    ...
    if not job_id:
        ...
        _update_status(tarefa_id, "falhou")
        record_event(hash_, "resumo_estruturado_erro", etapa="submit",
                         erro="resposta sem job_id")
        return

    record_event(hash_, "resumo_estruturado_job", job_id=job_id, total_steps=total_steps)
    _save_json(hash_, "resumo_estruturado_job.json", envio)
    ...
    while True:
        if time.time() - t0 > POLL_TIMEOUT:
            ...
            _update_status(tarefa_id, "falhou")
            record_event(hash_, "resumo_estruturado_erro", etapa="timeout", job_id=job_id)
            return
        ...
        if st.get("status") == "error":
            ...
            _update_status(tarefa_id, "falhou")
            record_event(hash_, "resumo_estruturado_erro", etapa="job",
                             erro=st.get("error"))
            return
    ...
    try:
        ...
        _update_status(tarefa_id, "falhou")
        record_event(hash_, "resumo_estruturado_erro", etapa="result", erro=str(e))
        return

    dados_llm = res.get("dados_llm")
    _save_json(hash_, "resumo_estruturado.json", dados_llm)
    if res.get("doc_text"):
        (folder(hash_) / "resumo_estruturado_texto.txt").write_text(
