...
| GET | `/tarefas/{id}/artefato/{nome}` (`:764-788`) | cookie + **propriedade** (`_e_dono`: fornecedor ou quem enviou; a cidadã vinculada não baixa artefato, porque `questoes.json` carrega o gabarito) | `nome` em `texto_extraido.txt, memoria_persistente.json, texto_tagueado.json, resumo_humanizado.md, questoes.json, log.jsonl, meta.json` ou qualquer nome começando com `T` (`:777-782`) | `FileResponse` chamado `{hash}_{nome}` | nenhum |
...
| Identidade de quem enviou (id, nome; o e-mail saiu) | `meta.json` (`app_gestao.py:179, 301, 432, 569`) | `workspace/{hash}/meta.json`, baixável por `/tarefas/{id}/artefato/meta.json` | usuários logados |
