...
export function AuthNav({ dark = true }: { dark?: boolean }) {
  ...
  const cls = `inline-flex min-h-[44px] items-center gap-1.5 whitespace-nowrap rounded-button px-2 text-[0.95rem] font-bold underline underline-offset-4 wide:px-2.5 ${dark ? "text-paper hover:bg-white/10" : "text-teal-deep hover:bg-teal-soft"}`;
  ...
  return (
    <nav aria-label="Sua conta" className="flex shrink-0 items-center gap-0.5 wide:gap-1">
      <span className={`hidden text-[0.95rem] wide:inline ${dark ? "text-paper/85" : "text-ink-2"}`}>{usuario.nome.split(" ")[0]}</span>
