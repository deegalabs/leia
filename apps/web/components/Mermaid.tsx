    ...
    <figure className="mb-4 rounded-card border border-line bg-surface p-3">
      ...
      <dialog ref={dialog} onCancel={(e) => { e.preventDefault(); close(); }} aria-label={label}
        ...
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between gap-2 border-b border-line px-3 py-2 wide:px-4">
            <p className="min-w-0 flex-1 truncate font-bold" title={label}>{label}</p>
            <div className="flex shrink-0 items-center gap-0.5 wide:gap-1">
              ...
              <button type="button" aria-label="Ajustar à tela" onClick={() => setZoom(1)} className="inline-flex min-h-[44px] min-w-[44px] items-center justify-center gap-1 rounded-button px-2 text-[0.9rem] font-bold text-teal-deep hover:bg-teal-soft"><Scan size={18} aria-hidden /><span className="hidden wide:inline">Ajustar</span></button>
              ...
              <button type="button" aria-label="Fechar" onClick={close} className="inline-flex min-h-[44px] min-w-[44px] items-center justify-center gap-2 rounded-button px-2 font-bold text-teal-deep hover:bg-teal-soft wide:px-3"><X size={18} aria-hidden /><span className="hidden wide:inline">Fechar</span></button>
            </div>
          </div>
